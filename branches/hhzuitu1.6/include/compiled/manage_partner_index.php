    ...
    <div id="content" class="coupons-box clear mainwide">
		<div class="box clear">
            ...
            <div class="box-content">
                <div class="head">
                    <h2>商户</h2>
					<ul class="filter"><li><form method="get">商户名称：<input type="text" name="ptitle" class="h-input" value="<?php echo htmlspecialchars($ptitle); ?>" >&nbsp;<select name="open" class="h-input"><?php echo Utility::Option($option_open, $open, '全部秀'); ?></select>&nbsp;<select name="city_id" class="h-input"><?php echo Utility::Option(option_category('city'), $city_id, '全部城市'); ?></select>&nbsp;<select name="group_id" class="h-input"><?php echo Utility::Option(option_category('partner'), $group_id, '全部分类'); ?></select>&nbsp;<input type="submit" value="筛选" class="formbutton"  style="padding:1px 6px;"/><form></li></ul>
				</div>
                <div class="sect">
					...
					<tr <?php echo $index%2?'':'class="alt"'; ?> id="team-list-id-<?php echo $one['id']; ?>">
						...
						<td style="text-align:left;"><a class="deal-title" href="/manage/partner/edit.php?id=<?php echo $one['id']; ?>"><?php if($one['head']){?><span style="color:red"><?php echo $one['title']; ?></span><?php } else { ?><?php echo $one['title']; ?><?php }?></a></td>
						...
						<td class="op" nowrap><a href="/manage/partner/edit.php?id=<?php echo $one['id']; ?>">编辑</a>｜<a href="/ajax/manage.php?action=partnerhead&id=<?php echo $one['id']; ?>" class="ajaxlink">置顶</a>｜<a href="/ajax/manage.php?action=partnerremove&id=<?php echo $one['id']; ?>" class="ajaxlink" ask="确定删除本商户？">删除</a></td>
					</tr>
					<?php }}?>
