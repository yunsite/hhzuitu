            ...
            <div class="box-content">
                ...
                <div class="sect">
					...
					<tr <?php echo $index%2?'':'class="alt"'; ?> id="team-list-id-<?php echo $one['id']; ?>">
						...
						<td><?php if($one['audit']==1){?>是<?php } else if($one['audit']==2) { ?>审核未通过<?php } else { ?>否<?php }?></td>
						<td class="op" nowrap><a href="/ajax/manage.php?action=teamdetail&id=<?php echo $one['id']; ?>" class="ajaxlink">详情</a>｜<a href="/manage/team/edit.php?id=<?php echo $one['id']; ?>">编辑</a>｜<a href="/ajax/manage.php?action=teamremove&id=<?php echo $one['id']; ?>" class="ajaxlink" ask="确定删除本项目吗？" >删除</a><?php if($one['close_time']&&in_array($one['state'],array('success','soldout'))){?>｜<a href="/manage/team/down.php?id=<?php echo $one['id']; ?>" target="_blank">下载</a><?php }?></td>
					</tr>
					<?php }}?>
