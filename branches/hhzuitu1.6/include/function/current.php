...
function current_biz() {
	...
	$a = array(
			'/biz/index.php' => '首页',
			'/biz/coupon_create.php' => '建立团购',
			'/biz/settings.php' => '商户资料',
			'/biz/coupon.php' => $INI['system']['couponname'] . '列表',
			);
	$r = $_SERVER['REQUEST_URI'];
	if (preg_match('#/biz/coupon.php',$r)) $l = '/biz/coupon.php';
	elseif (preg_match('#/biz/settings#',$r)) $l = '/biz/settings.php';
	elseif (preg_match('#/biz/coupon_create#',$r)) $l = '/biz/coupon_create.php';
...
function current_city($cename, $citys) {
	...
	return current_cigy_link($link, $links);
}

function current_cigy_link($link, $links, $span=false) {
	$html = '';
	$span = $span ? '<span></span>' : '';
	foreach($links AS $l=>$n) {
		if (trim($l,'/')==trim($link,'/')) {
			$html .= "<li style=\"float:left;width:80px;margin-bottom:5px;margin-left:5px;dborder:1px dashed #666;background:#cccccc;color:#f1f1f1;\"><a href=\"{$l}\" style=\"float:center\">{$n}</a>{$span}</li>";
		}
		else $html .= "<li style=\"float:left;width:80px;margin-bottom:5px;margin-left:5px;dborder:1px dashed #666;\"><a href=\"{$l}\" style=\"float:center\">{$n}</a>{$span}</li>";
	}
	return $html;
}
...
function mcurrent_team($selector=null) {
	$a = array(
		...
		'/manage/team/create.php' => '新建项目',
		'/manage/team/buy_num.php' => '购买人数修改',
	);
