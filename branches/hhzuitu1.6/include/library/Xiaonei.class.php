...
class Xiaonei {
	...
	{	
		...
		$this->session_key=$_COOKIE[$api_key.'_session_key'];
	...
	public function users($method,$array=array(),$format='XML')
	{
		...
		switch($method){
			case 'getInfo':
				...
				else $params['uids']=$_POST[$this->api_key.'_user'];
				if($array['fields'])
	...
	public function profile($method,$array=array(),$format='XML')
	{
		...
		switch($method){
			case 'getXNML':
				if($array['uid'])
				$params['uid']=$array['uid'];
				else $params['uid']=$_POST['c30f11542e8345239650ae7f621022f8_user'];break;
	...
	public function photos($method,$array=array(),$format='XML')
	{
		...
		switch($method){
			case 'getAlbums':
				if($array['uid'])
				$params['uid']=$array['uid'];
				else $params['uid']=$_POST['c30f11542e8345239650ae7f621022f8_user'];
				break;
